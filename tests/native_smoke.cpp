...
void check_lossless() {
	...
	require(tmb_encode_lossless(vertices.data(), 4, faces.data(), 4, TMB_VERTEX_IDS,
	...
	require(tmb_decode_lossless(encoded, encoded_size, decoded_vertices.data(),
	                               decoded_faces.data(), 4, 4, TMB_VERTEX_IDS,
	                               decode_timings) == TMB_OK);
    require(std::memcmp(vertices.data(), decoded_vertices.data(), sizeof(vertices)) == 0);
    require(canonical(faces.data()) == canonical(decoded_faces.data()));
    tmb_free(encoded);
}
...
void check_quantized(uint32_t bits) {
    ...
    size_t encoded_size = 0;
    double encode_timings[3] = {};
    require(tmb_encode_quantized(vertices.data(), 4, faces.data(), 4, TMB_VERTEX_IDS,
                                bits, &encoded, &encoded_size, encode_timings) == TMB_OK);
    std::array<float, 12> decoded_vertices = {};
    std::array<uint32_t, 12> decoded_faces = {};
    double decode_timings[2] = {};
    require(tmb_decode_quantized(encoded, encoded_size, decoded_vertices.data(),
                                decoded_faces.data(), 4, 4, TMB_VERTEX_IDS, bits,
                                decode_timings) == TMB_OK);
    const float tolerance = 62.125f / static_cast<float>((uint64_t{1} << bits) - 1) + 1e-5f;
    for (size_t i = 0; i < vertices.size(); ++i) {
        require(std::abs(vertices[i] - decoded_vertices[i]) <= tolerance);
    }
    require(canonical(faces.data()) == canonical(decoded_faces.data()));
    tmb_free(encoded);
}
